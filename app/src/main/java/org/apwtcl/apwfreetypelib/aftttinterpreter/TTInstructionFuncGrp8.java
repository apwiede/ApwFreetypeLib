...
public class TTInstructionFuncGrp8 extends FTDebug {
  ...
  public void SCANCTRL() {
    int A;
      ...
      /* Get Threshold */
    A = (cur.stack[cur.numArgs + 0] & 0xFF);
    if (A == 0xFF) {
    ...
    if ((cur.stack[cur.numArgs + 0] & 0x100) != 0 && cur.tt_metrics.getPpem() <= A) {
      cur.graphics_state.scan_control = true;
    }
    if ((cur.stack[cur.numArgs + 0] & 0x200) != 0 && cur.tt_metrics.isRotated()) {
      cur.graphics_state.scan_control = true;
    }
    if ((cur.stack[cur.numArgs + 0] & 0x400) != 0 && cur.tt_metrics.isStretched()) {
      cur.graphics_state.scan_control = true;
    }
    if ((cur.stack[cur.numArgs + 0] & 0x800) != 0 && cur.tt_metrics.getPpem() > A) {
      cur.graphics_state.scan_control = false;
    }
    if ((cur.stack[cur.numArgs + 0] & 0x1000) != 0 && cur.tt_metrics.isRotated()) {
      cur.graphics_state.scan_control = false;
    }
    if ((cur.stack[cur.numArgs + 0] & 0x2000) != 0 && cur.tt_metrics.isStretched()) {
      cur.graphics_state.scan_control = false;
    }
  }
  ...
  public void SDPVTL() {
    ...
    {
      FTVectorRec v1 = cur.zp1.getOrg()[cur.zp1.getOrg_idx() + p2];
      FTVectorRec v2 = cur.zp2.getOrg()[cur.zp2.getOrg_idx() + p1];
    ...
    {
      FTVectorRec v1 = cur.zp1.getCur()[cur.zp1.getCur_idx() + p2];
      FTVectorRec v2 = cur.zp2.getCur()[cur.zp2.getCur_idx() + p1];
...
//      GUESS_VECTOR( freeVector );
  ...
  public void GETINFO() {
    ...
    /********************************/
      ...
      /* Return Bit(s): 8             */
      /*                              */
    if ((cur.stack[cur.numArgs + 0] & 2) != 0 && cur.tt_metrics.isRotated()) {
      K |= 0x80;
    }
    /********************************/
      /* GLYPH STRETCHED              */
      ...
      /* Return Bit(s): 9             */
      /*                              */
    if ((cur.stack[cur.numArgs + 0] & 4) != 0 && cur.tt_metrics.isStretched()) {
      K |= 1 << 8;
    }
    /********************************/
  ...
  public void ROLL() {
    ...
    A = cur.stack[cur.numArgs + 2];
    B = cur.stack[cur.numArgs + 1];
    C = cur.stack[cur.numArgs + 0];
  ...
  public void SCANTYPE() {
    if (cur.stack[cur.numArgs + 0] >= 0) {
      cur.graphics_state.scan_type = (int)cur.stack[cur.numArgs + 0];
    }
  }
