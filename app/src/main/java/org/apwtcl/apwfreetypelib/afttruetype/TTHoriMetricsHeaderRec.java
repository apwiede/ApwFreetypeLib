...
public class TTHoriMetricsHeaderRec extends FTDebug {

  private static int oid = 0;

  private int id;
  private static String TAG = "TTHoriMetricsHeader";

  private int metrics_size = 0;
  private long metrics_offset = 0;

  /* ==================== TTHoriMetricsHeader ================================== */
  public TTHoriMetricsHeaderRec() {
    oid++;
    id = oid;
  }
...
Debug(0, DebugTag.DBG_LOAD_FACE, TAG, "Load: hmtx offset: "+String.format("0x%08x", stream.pos()));
    FTError.ErrorTag error = FTError.ErrorTag.ERR_OK;
    Integer length = new Integer(0);
    FTReference<Integer> length_ref = new FTReference<Integer>();

    length_ref.Set(length);
    error = ttface.gotoTable(TTTags.Table.hmtx, stream, length_ref);
